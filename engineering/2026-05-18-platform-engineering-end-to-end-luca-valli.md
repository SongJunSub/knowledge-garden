...
- *중간값 사용자 중심* = 가든 사용자는 *나 1명*. *중간값 = 나*. 그러나 *나의 중간값 시간(바쁜 평일)*에 가든이 동작하는가? — 답: *Ascetic 모드 실험이 정확히 그것에 대한 처방*
- *기초로 운영* = 가든의 *SLO*는 없음. *한 달 뒤 회고가 실제로 작동하는가*가 *가든의 SLO*. 7월 중순 검토에 명시.
